...
const runValidation = async (example: string, profile: string) => {
    const validator = new Validator(example, "RAML 1.0", "application/yaml", profile)
...
const runTest= async (name: string) => {
    ...
    assert.equal(JSON.stringify(positiveResult, null, 2), expectedPositive)
    ...
    assert.equal(JSON.stringify(negativeResult, null, 2), expectedNegative)
};
    ...
    files.map((f) => f.split(".")[0])/*.filter((p) => p == "profile2")*/.forEach((p) => profiles[p] = true);
