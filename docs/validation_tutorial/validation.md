...
This tutorial will review the custom validation mechanism that makes it possible to 
define, group, document, reuse and execute common validation rules over a semantic graph. By making use of AMF parsing capabilities rules can be defined for RAML, OAS and
...
When can parse this RAML file with the AMF command line: 
...
We can know invoke the custom validator passing the parsed JSON-LD graph and our custom profile which will enforce our defined validation.
...
AMF Custom Validator will return a JSON-LD graph which contains the resulting validation. You will find a `ValidationReport` node which contains information regarding the conformance, as well as a `ValidationResult` with information
regarding the 'version-format' validation which failed, its severity, and targeted node.
...
- `maxLength`: Validates the maximum length of the string value in a property of the target node can have
- `minLength`: Validates the minimum length of the string value in a property of the target node can have
- `exactLength`: Validates the exact length of the string value in a property of the target node can have
...
the standard description of the [API model](https://github.com/aml-org/amf/blob/develop/documentation/model.md) 
